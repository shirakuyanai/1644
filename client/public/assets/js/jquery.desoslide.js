(function ($, window, document, undefined) {
  ...
  Plugin.prototype = {
          ...
          if ($(item).find('img').length) {
            ...
            self.props.thumbs.push({
              ...
              caption_title:
                $(item)
                  .find('img')
                  .data(self._namespace + '-caption-title') || null,
              caption_link:
                $(item)
                  .find('img')
                  .data(self._namespace + '-caption-link') || null,
            });
      ...
      if (
        ...
        if (!this.props.effect.list.hasOwnProperty(effect.provider)) {
          ...
          if (effect.name === 'random') {
            ...
            if (
              !this.props.effect.list[effect.provider].hasOwnProperty(
                effect.name,
              )
            ) {
              response.provider = this._defaults.effect.provider;
    ...
    goTo: function (index) {
      if (
        $(this.options.thumbs).length > 1 &&
        this._isThumbExists(index) === true
      ) {
        var self = this;
    ...
    _isThumbExists: function (index) {
      ...
      if (typeof index === 'number') {
        if (this.props.thumbs[index] !== undefined) {
          ...
          this._errorHandler(
            'error',
            'The ' + index + "-indexed thumb doesn't exist.",
          );
              ...
              if (self.props.effect.list[key].hasOwnProperty(key2)) {
                if (classes.indexOf(self.props.effect.list[key][key2]) !== -1) {
                  this.props.img.$elem.removeClass(
                    self.props.effect.list[key][key2],
                  );
                }

                if (self.props.effect.list[key][key2].in) {
                  if (
                    classes.indexOf(self.props.effect.list[key][key2].in) !== -1
                  ) {
                    this.props.img.$elem.removeClass(
                      self.props.effect.list[key][key2].in,
                    );
                  }
                }
                ...
                if (self.props.effect.list[key][key2].out) {
                  if (
                    classes.indexOf(self.props.effect.list[key][key2].out) !==
                    -1
                  ) {
                    this.props.img.$elem.removeClass(
                      self.props.effect.list[key][key2].out,
                    );
                  }
                }
              }
    ...
    _getRandomEffect: function (provider) {
      ...
      for (prop in this.props.effect.list[provider]) {
        if (
          this.props.effect.list[provider].hasOwnProperty(prop) &&
          prop !== 'css'
        ) {
     ...
     */
    _hasEffect: function () {
      return this.props.effect.provider === null &&
        this.props.effect.name === 'none'
        ? false
        : true;
    },
      ...
      this.props.img.$elem
        ...
        .one('load', function () {
          ...
          if (self._hasEffect() === false) {
            ...
            if (self.props.is_transition_supported === true) {
              ...
              $(this)
                ...
                .removeClass(
                  self.props.effect.list[self.props.effect.provider].css +
                    ' ' +
                    self.props.effect.list[self.props.effect.provider][
                      self.props.effect.name
                    ].out,
                )
          ...
          if (self.options.auto.start === true) {
            self.props.img.timer = setTimeout(
              function () {
                self.goNext(true);
              },
    ...
    _overlay: function () {
      ...
      if (this.options.overlay !== 'none') {
        ...
        if (
          $(this.elem).find('.' + this._namespace + '-overlay').length === 0
        ) {
          $('<div>', {
        ...
        this.props.img.$overlay = $(this.elem).find(
        ...
        paddingBottom = parseInt(
          this.props.img.$overlay.css('padding-bottom'),
          10,
        );
        paddingLeft = parseInt(this.props.img.$overlay.css('padding-left'), 10);
        paddingRight = parseInt(
          this.props.img.$overlay.css('padding-right'),
          10,
        );

        overlayHeight =
          parseInt(this.props.img.$overlay.css('height'), 10) -
          (paddingLeft + paddingRight);
        overlayHeight =
          parseInt(height_plus_border, 10) -
          overlayHeight -
        ...
        if (
          this.options.controls.show === true &&
          this.props.thumbs.length > 1
        ) {
          this._controls();
        }
    ...
    _controls: function () {
      ...
      $pause =
        '<a class="' + this._namespace + '-controls pause" href="#pause"></a>';
      $play =
      ...
      this.props.controls.$wrapper = $(this.elem).find(
    ...
    _caption: function () {
      if (this.props.thumbs[this.props.img.to_show].caption_title !== null) {
        ...
        anchor_exists =
          this.props.img.$overlay.find('a:first').length > 0 ? true : false;
        href_exists =
          this.props.thumbs[this.props.img.to_show].caption_link !== null
            ? true
            : false;
    ...
    _triggerEvent: function (event_name) {
      ...
      var capitalize_first =
        event_name.charAt(0).toUpperCase() + event_name.slice(1);
    ...
    _events: function () {
      ...
      $(this.elem)
        ...
        .on({
          mouseover: function () {
            if (
              self.options.overlay === 'hover' &&
              self.props.img.$overlay !== null
            ) {
      ...
      switch (type) {
        case 'error':
          if (console !== undefined) {
            console.error(
              this._name + ': ' + msg + ' Check out the documentation.',
            );
          }
  ...
  $.fn[plugin_name] = function (options) {
    ...
    if (options === undefined || typeof options === 'object') {
      ...
      if (instance[options] !== undefined) {
        response = instance[options].apply(
          instance,
